...
fn main() {
    ...
    let mut mat1 = Tensor::random(1_000, 1_000, 42);
    let mut mat2 = Tensor::random(1_000, 1_000, 42);
    mat2.set_concurrent(false);

    let start1 = Instant::now();
    mat1.sigmoid();
    let duration1 = start1.elapsed();
    println!("Parallel tanh took: {:?}", duration1);

    let start2 = Instant::now();
    mat2.sigmoid();
    let duration2 = start2.elapsed();
    println!("Sequential tanh took: {:?}", duration2);
